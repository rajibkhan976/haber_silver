        ...
        <div class="col-sm-12">
            <div class="panel">
                ...
                <div class="panel-body">
                    {{-------------- Filter :Starts -------------------------------------------}}
                    {!! Form::open(['method' =>'GET','route'=>'admin.company.search']) !!}
                    <div id="index-search">
                        <div class="col-sm-3">
                            {!! Form::text('name',@Input::get('name')? Input::get('name') : null,['class' => 'form-control','placeholder'=>'type name', 'name'=>'type your require Company "name", example :: Abc, then click "search" button']) !!}
                        </div>
                    ...
                    <div class="table-primary">
                        <table cellpadding="0" cellspacing="0" border="0" class="table table-striped table-bordered" id="datatable1">
                            <thead>
                            <tr>
                                <th> Distributor's letterhead image </th>
                                <th> Company Name </th>                               
                                <th> Status </th>
                            ...
                            @if(isset($data))
                                @foreach($data as $values)
                                    <tr class="gradeX">
                                        <td> @if(isset($values->letter_head_thumb))<img src="{{ $values->letter_head_thumb }}"> @endif </td>
                                        <td>{{ucfirst($values->name)}} </td>
                                        <td>{{$values->status}}</td>
                                        <td>
                                            <a href="{{ route('admin.view.company', $values->id) }}" class="btn btn-info btn-xs" data-toggle="modal" data-target="#etsbModal" data-placement="top" data-content="view"><i class="fa fa-eye"></i></a>
                                            <a href="{{ route('admin.edit.company', $values->id) }}" class="btn btn-primary btn-xs" data-toggle="modal" data-target="#etsbModal" data-placement="top" data-content="update"><i class="fa fa-edit"></i></a>
                                            @if( $values->status !="cancel")
                                                <a href="{{ route('admin.delete.company', $values->id) }}" class="btn btn-danger btn-xs" onclick="return confirm('Are you sure to Delete?')" data-placement="top" data-content="delete"><i class="fa fa-trash-o"></i></a>
                                            @endif
                                        </td>
                                    </tr>
                                @endforeach
        ...
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                ...
                <div class="modal-body">
                     ...
                     {!! Form::open(['route' => 'admin.store.company','id' => 'user-jq-validation-form', "class"=>"form form-validate floating-label",  "novalidate"=>"novalidate", "enctype" => "multipart/form-data" ]) !!}
